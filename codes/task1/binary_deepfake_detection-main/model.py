...
class BNext4DFR(L.LightningModule):
    ...
    def accu(self, preds, pngs, threshold=0.5):
        ...
        # Map binary predictions to 'real' or 'fake'
        pred_labels = ['real' if pred == 1 else 'fake' for pred in binary_preds]
        
        # Prepare the data in JSON format
        results = [{"index": int(png), "prediction": label} for png, label in zip(pngs, pred_labels)]
        
        # Write results to a JSON file
        with open('41_task1.json', 'a') as f:
    ...
    def _step(self, batch, i_batch, phase=None):
        ...
        for i in range(len(batch['image_path'])):
            self.accu(outs['logits'][i], batch['image_path'][i].split('/')[-1])
